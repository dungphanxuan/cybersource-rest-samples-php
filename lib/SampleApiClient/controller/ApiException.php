    ...
    public function setExceptionLog($merchantConfig, $exception)
    {
        ...
        if ($merchantConfig->getLogConfiguration()->getEnableLogging()) {
            ...
            if (is_array($exception)) {
                error_log("[" . $date . "] [INFO] MERCHCFG:" . print_r($exception, true) . PHP_EOL, 3, $merchantConfig->getLogConfiguration()->getDebugLogFile());
            } else {
                error_log("[" . $date . "] " . $exception . PHP_EOL, 3, $merchantConfig->getLogConfiguration()->getDebugLogFile());
            }
        }
    }
}